...
class QEffMemoryVisualizer:
    ...
    def generate_professional_graph(self, filename: str) -> None:
        ...
        gs = fig.add_gridspec(3, 2, height_ratios=[2.5, 1.8, 1.2], width_ratios=[1, 1], 
                             hspace=0.35, wspace=0.2, left=0.05, right=0.98, top=0.90, bottom=0.08)
        ...
        fig.suptitle('QEfficient Enhanced Memory & Performance Analysis - Synchronized View', 
                    fontsize=18, fontweight='bold', color='#2E86AB', y=0.95)
    ...
    def _setup_memory_plot(self, ax, relative_times: List[float], rss_scaled: List[float], 
        ...
        # Main memory line
        ax.plot(relative_times, rss_scaled, color='#2E86AB', linewidth=3.5, 
                label='Memory Usage (RSS)', alpha=0.9, zorder=5)
        ...
        ax.set_title(f'Memory Usage Over Time | Peak: {peak_memory:.1f} {unit} | Duration: {total_duration:.1f}s', 
                    fontsize=14, fontweight='bold', color='#2E86AB', pad=15)
    ...
    def _setup_cpu_plot(self, ax, relative_times: List[float], cpu_usage: List[float]) -> None:
        ...
        if not relative_times or not cpu_usage or len(cpu_usage) != len(relative_times):
            ax.text(0.5, 0.5, 'CPU data not available or not synchronized', ha='center', va='center', 
                   transform=ax.transAxes, fontsize=12, color='#666666')
        ...
        # Main CPU line
        ax.plot(relative_times, cpu_usage, color='#FF6B35', linewidth=3, 
                label='CPU Usage', alpha=0.9, zorder=5)
        ...
        ax.axhline(y=avg_cpu, color='#E74C3C', linestyle='-', alpha=0.8, linewidth=2.5, 
                  label=f'Average: {avg_cpu:.1f}%', zorder=4)
        ...
        ax.set_title(f'CPU Usage Over Time | Peak: {max_cpu:.1f}% | Average: {avg_cpu:.1f}%', 
                    fontsize=14, fontweight='bold', color='#FF6B35', pad=15)
    ...
    def _setup_disk_io_plot(self, ax, sync_data: Dict[str, List[float]]) -> None:
        ...
        if not self.profiler.operations or len(self.profiler.operations) < 2:
            ax.text(0.5, 0.5, 'No operation phases available', ha='center', va='center', 
                   transform=ax.transAxes, fontsize=12, color='#666666')
        ...
        if not operations:
            ax.text(0.5, 0.5, 'No significant disk I/O detected', ha='center', va='center', 
                   transform=ax.transAxes, fontsize=12, color='#666666')
            ax.set_title('Disk I/O per Phase', fontsize=14, fontweight='bold')
            return
        ...
        bars_read = ax.bar(x_pos - bar_width/2, read_totals, bar_width, 
                          label='Read (MB)', color='#2196F3', alpha=0.8, 
                          edgecolor='white', linewidth=1.5)
        bars_write = ax.bar(x_pos + bar_width/2, write_totals, bar_width,
                           label='Write (MB)', color='#FF5722', alpha=0.8, 
                           edgecolor='white', linewidth=1.5)
        ...
        ax.text(0.02, 0.98, f'Total I/O: {total_read:.1f} MB read, {total_write:.1f} MB write', 
    ...
    def _setup_timing_plot(self, ax) -> None:
        ...
        ax.text(0.98, 0.02, f'Total: {total_duration:.1f}s', 
               transform=ax.transAxes, fontsize=10, va='bottom', ha='right',
               bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.9, 
                        edgecolor='gray', linewidth=1))
    
    def _draw_segment_backgrounds(self, ax, relative_times: List[float], 
                                 data_values: List[float], start_time: datetime, 
        ...
        for i in range(len(self.profiler.operations) - 1):
            ...
            rect = patches.Rectangle((op_start_time, 0), op_end_time - op_start_time, 
                                   max_value, linewidth=0, facecolor=color, 
                                   alpha=0.15, zorder=0)
            ax.add_patch(rect)
    ...
    def _draw_segment_boundaries(self, ax, start_time: datetime, max_value: float) -> None:
        ...
        for i, (op_time, op_name) in enumerate(self.profiler.operations):
            ...
            ax.axvline(x=boundary_time, color='#666666', linestyle='--', 
    ...
    def _mark_peak_memory(self, ax, start_time: datetime, scale: float, unit: str) -> None:
        ...
        ax.plot(peak_time_rel, peak_rss_scaled, 'o', color='#E74C3C', 
               markersize=14, markeredgecolor='white', markeredgewidth=3, 
               zorder=10, label='Peak Memory')
        
        # Enhanced annotation
        peak_text = f'Peak: {peak_rss_scaled:.1f} {unit}\nPhase: {self.profiler.peak_operation}'
        ax.annotate(peak_text, 
    ...
    def _add_segment_legend(self, ax) -> None:
        ...
        legend = ax.legend(handles=legend_elements, loc='upper left', 
                          bbox_to_anchor=(1.01, 1.0), fontsize=11,
                          title='QEfficient Phases', title_fontsize=12,
                          framealpha=0.95, edgecolor='#2E86AB', fancybox=True)
    ...
    def _calculate_io_per_phase(self, sync_data: Dict[str, List[float]]) -> Tuple[List[str], List[float], List[float]]:
        ...
        valid_operations = [(op_time, op_name) for op_time, op_name in self.profiler.operations 
    ...
    def _add_bar_labels(self, ax, bars_read, bars_write, read_totals: List[float], 
                       write_totals: List[float]) -> None:
        """Add value labels on bars."""
        max_val = max(max(read_totals) if read_totals else [0], max(write_totals) if write_totals else [0])
        
        for i, (read_bar, write_bar, read_val, write_val) in enumerate(zip(bars_read, bars_write, read_totals, write_totals)):
            if read_val > 0.01:
                ax.text(read_bar.get_x() + read_bar.get_width()/2, 
                       read_bar.get_height() + max_val * 0.02,
                       f'{read_val:.1f}', ha='center', va='bottom', 
                       fontsize=9, fontweight='bold', color='#2196F3')
            
            if write_val > 0.01:
                ax.text(write_bar.get_x() + write_bar.get_width()/2, 
                       write_bar.get_height() + max_val * 0.02,
                       f'{write_val:.1f}', ha='center', va='bottom', 
    ...
    def _add_duration_labels(self, ax, bars, durations: List[float]) -> None:
        ...
        for i, (bar, duration) in enumerate(zip(bars, durations)):
            ...
            ax.text(width + max_duration * 0.02, bar.get_y() + bar.get_height()/2, 
                   duration_text, ha='left', va='center', fontsize=10, fontweight='bold')
