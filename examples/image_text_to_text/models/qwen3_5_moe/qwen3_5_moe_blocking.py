...
def _compiled_qpc_path(compile_result, key, component):
    path = compile_result.get(key) if isinstance(compile_result, dict) else compile_result
    if not path:
...
GDN_CHUNK_SIZE = 1024
CTX_LEN = 14 * 1024
BATCH_SIZE = 8  # Per-slot prefill batch size
BS = BATCH_SIZE
FULL_BATCH_SIZE = BATCH_SIZE  # Total concurrent CB slots
...
# Must run before qeff_model.compile() so the traced chunk size matches the
# registered _mask_causal/_mask_strict/_ones_lower/_eye buffers.
...
# )
os.environ["QAIC_COMPILER_OPTS_UNSUPPORTED"] = "-aic-op-stats-verbosity 2 -aic-hoist-vtcm-loads=false "
...
if args.decode_qpc_path:
    ...
    decode_qpc_path = _compiled_qpc_path(decode_compile_result, "lang_decode_qpc_path", "decode")
