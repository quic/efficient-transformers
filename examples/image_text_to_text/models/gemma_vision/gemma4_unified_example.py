...
SKIP_VISION = os.getenv("QEFF_SKIP_VISION", "0") == "1"
BS = int(os.getenv("QEFF_BATCH_SIZE", "1"))
PREFILL_SEQ_LEN = int(os.getenv("QEFF_PREFILL_SEQ_LEN", "128"))
CTX_LEN = int(os.getenv("QEFF_CTX_LEN", "2048"))
GENERATION_LEN = int(os.getenv("QEFF_GENERATION_LEN", "1024"))

NUM_LANG_HIDDEN_LAYER = int(os.getenv("QEFF_NUM_LANG_HIDDEN_LAYER", "6"))
...
def _apply_reduced_layer_config(config, num_lang_layers: int):
    ...
    if hasattr(config.text_config, "layer_types") and config.text_config.layer_types:
        ...
        if config.text_config.layer_types[-1] != "full_attention":
            config.text_config.layer_types[-1] = "full_attention"
...
def main():
    ...
    # config = _apply_reduced_layer_config(config, NUM_LANG_HIDDEN_LAYER)
    ...
    if SKIP_VISION:
        ...
        effective_prefill_seq_len, effective_ctx_len = effective_lens(
            qeff_model, PREFILL_SEQ_LEN, CTX_LEN, prompt_len, GENERATION_LEN, SKIP_VISION
        )
    ...
    qeff_model.compile(**compile_kwargs)
    output = qeff_model.generate(inputs=inputs, generation_len=GENERATION_LEN)
    qeff_ids = normalize_generated_ids(output.generated_ids)[:, :GENERATION_LEN]
    print(tokenizer.batch_decode(qeff_ids, skip_special_tokens=True))
    print(output)
