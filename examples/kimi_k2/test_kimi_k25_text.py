...
def test_qeff_generation(qaic_config):
    ...
    kimi_cls = get_class_from_dynamic_module("modeling_deepseek.DeepseekV3ForCausalLM", str(MODEL_PATH))
    ...
    # Recompute reference logits after dtype conversion so we compare
    # transformed fp32 path against fp32 reference.
    with torch.no_grad():
        out = model(**inputs)

    qeff_model = QEFFAutoModelForCausalLM(model, qaic_config=qaic_config)
    qeff_model.transform(ctx_len=CTX_LEN, seq_len=PREFILL_SEQ_LEN, bs=1, num_devices=1, qaic_config=qaic_config)
    ...
    x = prefill_qeff_out.logits
    y = out.logits[:, -1, :]
    assert (x - y).abs().max() < 2e-4
