...
def draft_spec_decode_inference(
    ...
    assert decode_batch_size == 1, "PLD execution only supports batch size of 1."
    ...
    tlm_position_ids = np.arange(num_speculative_tokens + 1).reshape(1, -1).repeat(decode_batch_size, 1)
    input_ids = np.zeros((decode_batch_size, ctx_len + input_lengths[0]), dtype=np.int64)
    prompt_and_prefill_tokens = np.concatenate(
        [prompts_tokenized[0]["input_ids"][:, : input_lengths[0]], tlm_precode_inputs["input_ids"][:, :1]], axis=1
    )
    input_ids[:, : input_lengths[0] + 1] = prompt_and_prefill_tokens
    prompt_plus_gen_idx = input_lengths[0] + 1
    while True:
        it += 1
        ...
        if has_empty_tokens:
            ...
            tlm_precode_inputs["position_ids"][:, 1:] = -1
        else:
            tlm_precode_inputs["input_ids"][:, 1:] = spec_tokens
        ...
        if has_empty_tokens:
            ...
            tlm_precode_position_ids = tlm_position_ids + (tlm_precode_inputs["position_ids"][:, 0] + 1)
        else:
            ...
            tlm_precode_position_ids = tlm_precode_inputs["position_ids"] + num_tokens_selected
        ...
        for bi, valid in enumerate(valid_batch_indices):
            ...
            input_ids[bi, prompt_plus_gen_idx : prompt_plus_gen_idx + num_tokens_to_append] = gen_ids
            prompt_plus_gen_idx += num_tokens_to_append
...
def arg_parse():
    ...
    parser.add_argument("--prompts", type=str, nargs="+", default=default_prompt, help="Input prompt(s)")
