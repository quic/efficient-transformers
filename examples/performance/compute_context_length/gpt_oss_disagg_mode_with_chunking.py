...
qeff_model1 = QEFFAutoModelForCausalLM.from_pretrained(
    model_id,
    qaic_config={
        "ccl_enabled": True,
    },
...
comp_ctx_lengths_prefill = [1024, 2048, 4096]
...
if comp_ctx_lengths_prefill is not None:
    list_of_comp_ctx_lengths_prefill = [np.zeros(length, dtype=np.int8) for length in comp_ctx_lengths_prefill]
    prefill_ccl_id = 0
...
for i in range(num_chunks):
    ...
    if comp_ctx_lengths_prefill is not None:
        if (i + 1) * PREFILL_SEQ_LEN > comp_ctx_lengths_prefill[prefill_ccl_id]:
            prefill_ccl_id = min(prefill_ccl_id + 1, len(comp_ctx_lengths_prefill) - 1)
