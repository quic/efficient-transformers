...
def _export_compile_audio_model(model_name, get_pipeline_config, audio_model_artifacts, torch_dtype, dtype_key="fp32"):
    ...
    audio_model_artifacts.setdefault(model_name, {}).setdefault(dtype_key, {}).update(
        {
...
def test_export_compile_audio_model(model_name, get_pipeline_config, audio_model_artifacts):
    """FP32 export + compile."""
    _export_compile_audio_model(model_name, get_pipeline_config, audio_model_artifacts, torch.float32)


# Config 2: FP16
# xfail: AI100 compiler does not support Clip op in float16 (MODEL_LOADER_UNSUPPORTED_DATATYPE)
@pytest.mark.xfail(reason="AI100 compiler does not support Clip op in float16", strict=False)
@pytest.mark.parametrize("model_name", test_models)
def test_export_compile_audio_model_fp16(model_name, get_pipeline_config, audio_model_artifacts):
    """FP16 export + compile."""
    _export_compile_audio_model(model_name, get_pipeline_config, audio_model_artifacts, torch.float16, dtype_key="fp16")
