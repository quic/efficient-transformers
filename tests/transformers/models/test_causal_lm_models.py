...
@pytest.mark.parametrize("model_name", [conf['model_name'] for conf in json.load(open(TEST_CONFIG_FILE_PATH, "r"))['models']], ids=lambda x: "model_name="+ str(x))
class TestQEfficientModels:
    ...
    def test_qefficient_model_torch(self, model_name):
        ...
        """
        (self.setup_infos[model_name]["pytorch_hf_tokens"] == self.setup_infos[model_name]["pytorch_kv_tokens"]).all(), "Tokens don't match for HF PyTorch model output and KV PyTorch model output"
    ...
    def test_qefficient_model_onnx(self, model_name):
        ...
        :param model_name: Name of model.
        """
        (self.setup_infos[model_name]["pytorch_kv_tokens"] == self.setup_infos[model_name]["ort_tokens"]).all(), "Tokens don't match for ONNXRT output and PyTorch output."
    ...
    def test_qefficient_model_cloud_ai_100(self, model_name):
        ...
        (self.setup_infos[model_name]["ort_tokens"] == cloud_ai_100_tokens).all(), "Tokens don't match for ONNXRT output and Cloud AI 100 output."
