    ...
    *,
    ...
    if enable_chunking:
        ...
        for i in range(num_chunks):
            ...
            chunk_inputs["position_ids"] = qeff_inputs["position_ids"][
...
def _run_qpc_prefill(
    ...
    if enable_chunking:
        ...
        for i in range(num_chunks):
            ...
            chunk_inputs["position_ids"] = qeff_inputs_np["position_ids"][
                :, i * prefill_seq_len : (i + 1) * prefill_seq_len
            ]
    ...
    for i in range(config.num_hidden_layers):
        if i % 2 == 0 and decode_inputs["position_ids"] >= config.sliding_window:
            ...
            decode_inputs[f"past_value.{i}"] = np.concatenate(
                (v[:, :, rot_offset:, :], v[:, :, :rot_offset, :]), axis=-2
            )
        else:
...
def _prefix_caching_inference(
    ...
    qpc_out, inputs = _run_qpc_prefill(
        inputs, prefill_qpc_path, config, prefill_seq_len, enable_chunking=True, num_chunks=num_chunks
    )
