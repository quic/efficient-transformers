...
def test_fp16_export_compile_kv_replicate(model_name):
    """Verify that FP16 export and compilation succeed with KV-head replication.

    Derives the replication factor from the model config
    (``num_attention_heads / num_key_value_heads``), applies
    ``ReplicateKVHeadTransform`` to expand GQA KV heads to match the query head
    count, exports to ONNX in FP16, and compiles to a QPC.  Asserts that
    ``qconfig.json`` is present.  Inference is not run.
...
@pytest.mark.qaic
@pytest.mark.parametrize("model_name", test_models_causal)
def test_fp16_export_compile_generate_kv_replicate(model_name):
    """Verify end-to-end FP16 inference quality with KV-head replication.

    Applies ``ReplicateKVHeadTransform``, exports in FP16, compiles to a QPC,
    runs inference on the AIC device, and checks that the cosine similarity
    between the AIC output token sequences and the HF PyTorch reference sequences
