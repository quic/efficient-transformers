...
"""Real-weight parity test for ``glm_moe_dsa``.

Loads a layer-and-expert-trimmed slice of ``zai-org/GLM-5.1`` with real Hub
weights via ``runners.real_weight_tiny.load_trimmed_real_weights``, runs a
CPU forward through HF and through the QEff PyTorch path, and asserts
logit-level parity (cosine >= 0.99, relative L2 <= 5e-2).

The dummy-weight ``test_glm_moe_dsa_tiny.py`` validates structure only;
this test catches numerical regressions in:
- MLA Q/K/V split + RoPE on the rope slice.
- Sigmoid + e_score_correction_bias routing (selection scores vs weight scores).
- Top-k einsum aggregation across experts.
...
TRIM_LAYERS = 4  # >= first_k_dense_replace=3, so we exercise at least one MoE layer
TRIM_EXPERTS = 8
...
def test_real_weight_logit_parity(trimmed_model):
    ...
    metrics = assert_logit_parity(hf_out, qeff_out, label="glm_moe_dsa trimmed", min_cosine=0.99, max_relative_l2=5e-2)
