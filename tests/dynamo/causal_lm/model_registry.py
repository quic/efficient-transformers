...
class DynamoModelSpec:
    ...
    # Weight-free export support
    weight_free_supported: bool = True
    weight_free_cb_supported: bool = True
...
DYNAMO_MODEL_SPECS: List[DynamoModelSpec] = [
    ...
    DynamoModelSpec(
        ...
        "vlm_text",
        weight_free_supported=False,
        weight_free_cb_supported=False,
    ),
...
def specs_with(
    *,
    subfunctions: bool = False,
    continuous_batching: bool = False,
    weight_free: bool = False,
    weight_free_cb: bool = False,
) -> List[DynamoModelSpec]:
    ...
    for spec in DYNAMO_MODEL_SPECS:
        ...
        if continuous_batching and not (spec.continuous_batching_supported and spec.model_loader == "causal_lm"):
            continue
        if weight_free and not spec.weight_free_supported:
            continue
        if weight_free_cb and not (spec.weight_free_cb_supported and spec.model_loader == "causal_lm"):
            continue
        result.append(spec)
    return result
