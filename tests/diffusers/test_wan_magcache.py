...
from QEfficient.diffusers.pipelines.wan.magcache import WanMagCacheRuntime, nearest_interp
...
def test_prepare_ratios_cfg_and_non_cfg_lengths():
    ...
    cfg_runtime = WanMagCacheRuntime(
    ...
    non_cfg_runtime = WanMagCacheRuntime(
        num_inference_steps=5,
...
@pytest.mark.diffusers
def test_stage_aware_retention_window_behavior():
    ...
    assert allowed == [False, True, True, False, True]
...
def test_skip_path_advances_call_index_and_respects_k_limit():
    ...
    # End of denoise sequence resets runtime state for next video.
