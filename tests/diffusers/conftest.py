...
Under the per-PR tiny lane (QEFF_TEST_PROFILE in {dummy_layers_model,
few_layers_model}), FLUX.1-schnell is gated and cannot be accessed without HF
auth. This conftest swaps the module-level constant in test_flux.py to a public
tiny substitute at collection time so no auth token is needed.
"""
...
def pytest_configure(config):
    ...
    if profile not in {"dummy_layers_model", "few_layers_model"}:
        return

    try:
        from tests.utils.tiny_overrides import resolve_model_id
    except Exception:
        return
