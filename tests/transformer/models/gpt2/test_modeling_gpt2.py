...
def get_config():
    ...
    model_config["model_class"] = GPT2LMHeadModel
    model_config["n_layer"] = config.n_layer
    model_config["padding_shape"] = [1, n_heads, Constants.CTX_LEN, d_head]
    return model_config


class TestQEfficientGPT2(unittest.TestCase):
    @classmethod
    def setUpClass(self):
        """
        Set up function to set up the test environment for TestQEfficientGPT2 class
        :param None
        """
        self.model_config = get_config()
        self.setup_info = set_up(self.model_config)

    def test_qefficient_gpt2_torch(self):
        """
        Test function to validate the gpt2 model before and after KV changes on Pytorch
        :param None
        """
        assert (
    ...
    def test_qefficient_gpt2_onnx(self):
        """
        Test function to validate the gpt2 model before and after KV changes on ONNXRT
        :param None
        """
        assert (
    ...
    def test_qefficient_gpt2_cloud_ai_100(self):
        """
        Test function to validate the gpt2 model before and after KV changes on Cloud AI 100
        :param None
        """
