...
* HuggingFace model files Download → Optimize for Cloud Cloud AIxxx (AI100, AI200 and so on) → Export to `ONNX` → Compile on Cloud Cloud AIxxx (AI100, AI200 and so on) → [Execute](#execute_api)
...
Here is the high level API to compile and run the model on Cloud AIxxx (AI100, AI200 and so on) via Python using Qeff Autoclasses.
To Know more about the QEFF Auto Classes, refer the link [QEFFAutoClasses](qeff_autoclasses.md)


### 1. Model download and Optimize for Cloud AIxxx (AI100, AI200 and so on)
...
print(f"{model_name} optimized for Cloud AIxxx (AI100, AI200 and so on) \n", qeff_model)
```
...
# Cloud AIxxx (AI100, AI200 and so on) Platform.
...
Benchmark the model on Cloud AIxxx (AI100, AI200 and so on), run the infer API to print tokens and tok/sec
