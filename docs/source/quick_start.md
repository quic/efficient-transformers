...
For QNN Compilation, add --enable_qnn in the command and an optional config file if user wish to override the default parameters.
Without QNN Config
```bash
...
For QNN Compilation, add --enable_qnn in the command and an option config file if user wish to override the default parameters.
```bash
