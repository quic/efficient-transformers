...
If QEfficient is already installed, install `torch_qaic` and (optionally) `accelerate` for QAIC:
...
> **Note**  
> If you’re using the `qeff-env` Docker environment, `torch_qaic` and `accelerate` may already be installed.
...
### Using a Custom Dataset (Registry)
...
class MyCustomDataset(Dataset):
    def __init__(self, path: str, prompt_template: str = None):
        self.rows = [json.loads(l) for l in open(path, "r", encoding="utf-8")]
        self.template = prompt_template or "### Instruction:\n{prompt}\n### Response:\n{response}"
    ...
    def __getitem__(self, idx):
        row = self.rows[idx]
        # Safely format the prompt using the template placeholders.
        # Expect keys like 'prompt' and 'response' in the JSON lines.
...
> **Tip**: Ensure your JSON rows have keys that match the placeholders used in `prompt_template`.

### Using a Hugging Face Dataset with a Template Function
...
data:
  ...
  split: "train"
  prompt_template: "alpaca_func:format_alpaca"
```
...
```bash
torchrun --nproc_per_node=4 finetune.py --config configs/distributed_config.yaml
```

**Distributed (Accelerate)**

```bash
accelerate launch --num_processes 4 finetune.py --config configs/distributed_config.yaml
```
