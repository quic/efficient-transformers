...
model_name = "BAAI/bge-large-en-v1.5"  # Similar, we can change model name and generate corresponding models, if we have added the support in the lib.
qeff_model = AutoModel.from_pretrained(pretrained_model_name_or_path="BAAI/bge-large-en-v1.5", add_pooling_layer=False)
...
embd= qeff_model.generate(tokenizer=tokenizer,prompt=["My name is"], qpc_path="/local/mnt/workspace/amitraj/.cache/qeff_models/BertModel-92d7022e45391d95/qpc-6e50deef13b55727")
...
np.mean(np.abs(embd_output - pt_output))
mad =  np.mean(np.abs(embd['output'] - pt_outputs2[0].numpy))
