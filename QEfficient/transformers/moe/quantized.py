...
def _cumsum_scatter_gather_update_quantized_expert(
    ...
    """Execute one UInt4 expert-parallel slot using packed active tokens."""
    batch_size, seq_len = token_to_expert.shape
    ...
    matched_indices = _build_matched_idx_from_cumsum(token_to_expert)
    ...
    expanded_x = x.unsqueeze(0).expand(batch_size, -1, -1)

    for chunk_index in range(num_packed_chunks):
        packed_start = chunk_index * packed_chunk_size
        packed_stop = packed_start + packed_chunk_size
        chunk_indices = matched_indices[:, packed_start:packed_stop]
        x_chunk = CtxGatherFunc3DGeneralized.apply(expanded_x, chunk_indices)

        gate_proj = _dequantize_projection(gate_qweight, gate_scales, gate_qzeros, group_size)
        up_proj = _dequantize_projection(up_qweight, up_scales, up_qzeros, group_size)
        down_proj = _dequantize_projection(down_qweight, down_scales, down_qzeros, group_size)

        gate_out = torch.bmm(x_chunk, gate_proj.transpose(1, 2).to(x_chunk.dtype))
        up_out = torch.bmm(x_chunk, up_proj.transpose(1, 2).to(x_chunk.dtype))
        down_out = torch.bmm(act_fn(gate_out) * up_out, down_proj.transpose(1, 2).to(x_chunk.dtype))

        routing_chunk = CtxGatherFunc3DGeneralized.apply(routing_weight, chunk_indices)
        previous_out = CtxGatherFunc3DGeneralized.apply(expert_out, chunk_indices)
        ...
        current_out = (
            torch.where((row_range < chunk_valid_rows).unsqueeze(-1), down_out, torch.zeros_like(down_out))
            * routing_chunk
        )
        updated_chunk = previous_out + current_out
        expert_out = CtxScatterFunc3DGeneralized.apply(expert_out, chunk_indices, updated_chunk)
