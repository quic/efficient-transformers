...
def gptoss_clamped_glu_mlp(
    ...
    *,
    ...
    gate = gptoss_clamp(gate, min_value=torch.finfo(torch.float16).min, max_value=limit)
    up = gptoss_clamp(up, min_value=-limit, max_value=limit)
...
def gptoss_clamp(
    ...
    *,
    ...
    """Clamp without exporting ONNX ``Clip`` nodes.

    QAIC AI100 does not support the element type emitted for the ONNX Clip
    nodes generated by ``torch.clamp`` in the GPT-OSS expert path.  Materialize
    scalar bounds in the input dtype and use ``Maximum``/``Minimum`` instead.
    This preserves clamp semantics while keeping all operands dtype-consistent.
    """
    if min_value is not None:
        min_tensor = x.new_full((), min_value)
        x = torch.maximum(x, min_tensor)
    if max_value is not None:
        max_tensor = x.new_full((), max_value)
