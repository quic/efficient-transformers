...
class QEffDynamicCache(DynamicCache):
    ...
    def update(
        ...
        if len(self.key_cache) <= layer_idx:
            ...
            invalid_mask = (ctx_indices > gather_limit) | (ctx_indices >= CCL)
            ...
            ctx_indices = ctx_indices[:, :, :CCL]
            invalid_mask = invalid_mask[:, :, :CCL]

            ctx_indices = torch.where(invalid_mask, invalid_idx_value, ctx_indices)

            if batch_index is not None:
                k_out = CtxGatherFuncCB.apply(self.key_cache[layer_idx], batch_index, ctx_indices, CCL)
                v_out = CtxGatherFuncCB.apply(self.value_cache[layer_idx], batch_index, ctx_indices, CCL)
            else:
                k_out = CtxGatherFunc.apply(self.key_cache[layer_idx], ctx_indices, CCL)
                v_out = CtxGatherFunc.apply(self.value_cache[layer_idx], ctx_indices, CCL)
