...
Today only ``qwen3_vl_moe``, ``qwen3_5_moe``, ``qwen3_moe`` and
``minimax_m3_vl`` carry the windowing hooks (``_start``/``_end`` class attributes) that this driver relies
...
def _install_window_patches_for(model_type: str) -> None:
    ...
    if minimax_mod is not None and model_type in {"minimax_m3_vl", "minimax_m3_vl_text"}:
        candidates.extend(
            cls
            for name in ("MiniMaxM3VLForCausalLM", "MiniMaxM3SparseForConditionalGeneration")
