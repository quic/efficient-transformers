...
class QEffQwen3_5MoeGatedDeltaNet(Qwen3_5MoeGatedDeltaNet):
    ...
    def _recurrent_step_batched(
        self, query, key, value, g, beta, recurrent_state, num_batch_blocks: int = 1
    ):
        """
        ...
        num_blocks = max(1, min(num_heads, int(num_batch_blocks)))
        ...
        for block_idx in range(num_blocks):
            ...
            q_block = q[:, :, head_start:head_end].reshape(
                1, folded_batch_heads, sequence_length, key_head_dim
            )
            k_block = k[:, :, head_start:head_end].reshape(
                1, folded_batch_heads, sequence_length, key_head_dim
            )
            v_block = v[:, :, head_start:head_end].reshape(
                1, folded_batch_heads, sequence_length, value_head_dim
            )
            ...
            state_block = state[:, head_start:head_end].reshape(
                1, folded_batch_heads, key_head_dim, value_head_dim
            )
            ...
            output_blocks.append(
                block_output.reshape(batch_size, block_heads, sequence_length, value_head_dim)
            )
            state_blocks.append(updated_state.reshape(batch_size, block_heads, key_head_dim, value_head_dim))

        output = torch.cat(output_blocks, dim=1).reshape(
    ...
    def forward(
        ...
        linear_attn_num_batch_blocks: int = 1,
    ):
        ...
        if cache_params is not None:
            if seq_len == 1:
                ...
                core_attn_out, last_recurrent_state = self._recurrent_step_batched(
                    ...
                    beta,
                    recurrent_state,
                    num_batch_blocks=linear_attn_num_batch_blocks,
                )
            else:
        ...
        **kwargs,
        ...
        if self.layer_type == "linear_attention":
            hidden_states = self.linear_attn(
                ...
                linear_attn_num_batch_blocks=linear_attn_num_batch_blocks,
            )
        else:
...
class QEffQwen3_5MoeDecoderWrapper(nn.Module):
    ...
    def forward(
        ...
        batch_fold_cb = batch_index is not None and blocking_config is not None and bool(blocking_config.batch_fold)
        linear_attn_num_batch_blocks = (
            max(1, int(blocking_config.num_batch_blocks or 1)) if batch_fold_cb else 1
        )
