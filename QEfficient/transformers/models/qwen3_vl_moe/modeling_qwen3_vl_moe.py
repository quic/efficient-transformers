...
def _cumsum_scatter_gather_update_qwen3vlmoe_expert_blocked(
    x: torch.Tensor,
    T2Ei: torch.Tensor,
    W_g: torch.Tensor,
    W_u: torch.Tensor,
    ...
    """Cumsum-scatter-gather-update expert helper for Qwen3-VL-MoE NSP-blocked dispatch.
...
class QEffPrefillChunkedQwen3VLMoeTextSparseMoeBlock(Qwen3VLMoeTextSparseMoeBlock):
    """NSP-blocked prefill dispatch for Qwen3-VL-MoE text sparse MoE block."""

    supports_moe_prefill_blocking = True

    def __qeff_init__(self):
        # Split fused gate_up_proj [E, H, 2I] into separate gate and up [E, H, I]
    ...
    def _forward_expert_blocked(
        ...
        for slot in range(local_experts):
            ...
            expert_out = _cumsum_scatter_gather_update_qwen3vlmoe_expert_blocked(
                x=x,
                T2Ei=T2Ei,
                W_g=W_g[:, slot],
                W_u=W_u[:, slot],
    ...
    def orig_forward(self, hidden_states: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for e in range(self.num_experts):
            ...
            expert_out += torch.where(routing_weight > 0, down * routing_weight, torch.zeros_like(expert_out))
