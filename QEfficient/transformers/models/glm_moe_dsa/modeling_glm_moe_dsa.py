...
n_group=1 collapses to global top-k) **with the DSA (DeepSeek Sparse Attention)
indexer**. The indexer keeps its own key cache, scores every cached key, and
selects a sparse top-``index_topk`` set per query (``GlmMoeDsaIndexer`` + additive
sparse mask), so this build can serve long context — not just the short-ctx
dense-masked simplification.

GLM-5.2 keeps the GLM-5.1 layer math and adds a cross-layer **shared-indexer
schedule** (``config.indexer_types``: one ``"full"`` indexer every 4th layer —
``index_topk_freq=4`` — the next three ``"shared"`` layers reuse its top-k; HF
sets ``self.indexer = None`` on shared layers) plus ``rope_theta=8e6`` and a 1M
...
from QEfficient.transformers.models.glm4_moe.modeling_glm4_moe import _cumsum_scatter_gather_update_expert_blocked
from QEfficient.utils.constants import MIN_MASKED_ATTENTION_VALUE


def apply_rotary_pos_emb(x, cos, sin, unsqueeze_dim=1):
    """Single-tensor rotary embedding.

    transformers 5.12.1 only ships the two-tensor ``apply_rotary_pos_emb(q, k, cos, sin)``
    form, but the GLM-MoE-DSA attention rotates ``q_pe`` and ``k_pe`` independently. This
    helper preserves the single-tensor contract the modeling relies on.
    """
...
class QEffPrefillChunkedGlmMoeDsaMoE(QEffGlmMoeDsaMoE):
    ...
    kernel (same expert weight layout). Used by ``pytorch_transforms`` for the disaggregated
    prefill-chunking path."""
