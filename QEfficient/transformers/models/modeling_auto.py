...
class QEFFAutoModelForImageTextToText(QEFFTransformersBase):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if kwargs.pop("full_batch_size", None):
            raise NotImplementedError("Continuous batching is not supported for image-text-to-text models yet.")
        if pretrained_model_name_or_path == "OpenGVLab/InternVL2_5-1B":
            cls._hf_auto_class = AutoModelForCausalLM
        ...
        self = super().from_pretrained(pretrained_model_name_or_path, is_tlm=is_tlm, config=config, *args, **kwargs)
        self.continuous_batching = continuous_batching

        if pretrained_model_name_or_path == "OpenGVLab/InternVL2_5-1B":
            tokenizer = AutoTokenizer.from_pretrained(
                pretrained_model_name_or_path, trust_remote_code=True, use_fast=False
            )
            self.processor = InternProcessor(self.model, tokenizer)
        else:
            self.processor = AutoProcessor.from_pretrained(
                pretrained_model_name_or_path, padding_side="right", trust_remote_code=True
            )
    ...
    def export(
        ...
        **kwargs,
        ...
        example_inputs, dynamic_axes, output_names = self.model.generate_inputs(self.processor, **kwargs)
        return self._export(
            example_inputs,
            output_names,
            dynamic_axes,
    ...
    def cloud_ai_100_vlm_generate(
        ...
        inputs["input_ids"] = torch.nn.functional.pad(
            inputs["input_ids"],
            (0, Constants.SEQ_LEN_VLM - input_ids_size),
            "constant",
            self.processor.tokenizer.pad_token_id,
        )
        inputs["attention_mask"] = torch.nn.functional.pad(
            inputs["attention_mask"], (0, Constants.SEQ_LEN_VLM - input_ids_size), "constant", 0
        )
