...
class QEFFAutoModelForCausalLM(QEFFBaseModel):
    ...
    #     dynamic_axes: Dict[str, Dict[int, str]],
    # ) -> dynamic_shapes_api.ShapesCollection:
    #     """
    #     Convert torch.onnx-style dynamic_axes to a torch.export ShapesCollection.

    #     Parameters
    #     ----------
    #     example_inputs : Dict[str, Any]
    #         The same example_inputs dict used for export. Keys here should
    #         correspond to the names in dynamic_axes (e.g. "input_ids",
    #         "position_ids", "past_key.0", etc.), so we can map from names
    #         to actual tensors.

    #     dynamic_axes : Dict[str, Dict[int, str]]
    #         Mapping from tensor name to a dict {dim_index: dim_name}, as
    #         used in torch.onnx.export.
    ...
    #     -------
    #     ShapesCollection
    #         A ShapesCollection suitable for the `dynamic_shapes` argument
    #         to torch.export, keyed by the actual tensors in example_inputs.
    #     """

    #     # Cache Dim objects by name so same name => same symbolic dimension
    #     dim_cache: Dict[str, Dim] = {}

    #     # def get_dim(name: str) -> Dim:
    #     #     if name not in dim_cache:
    #     #         # You can add min/max here if needed: Dim(name, min=1, max=...)
    #     #         dim_cache[name] = Dim(name)
    #     #     return dim_cache[name]

    #     def get_dim(name: str) -> Dim:
    #         if name not in dim_cache:
    #             # Patch: special min/max for specific dimension names
    #             if name == "batch_size":
    #                 # Example: allow batch_size from 1 to 1024
    #                 dim_cache[name] = Dim("batch_size", min=1, max=1024)
    #             elif name == "seq_len":
    #                 # Example: disallow tiny seq_len if needed; adjust as per model
    #                 # Here: seq_len between 2 and 4096 (tweak as appropriate)
    #                 dim_cache[name] = Dim("seq_len", min=2, max=4096)
    #             elif name == "ctx_len":
    #                 # Example: context length from 1 to 8192
    #                 dim_cache[name] = Dim("ctx_len", min=1, max=8192)
    #             else:
    #                 # Default: no explicit bounds
    #                 dim_cache[name] = Dim(name)
    #         return dim_cache[name]

    #     # Create a ShapesCollection
    #     shapes = dynamic_shapes_api.ShapesCollection()

    #     # Iterate over each name in dynamic_axes
    #     for name, axes_spec in dynamic_axes.items():
    #         if name not in example_inputs:
    #             # If your naming doesn’t match 1:1, you’ll need custom mapping here.
    #             # For now, we just skip and optionally log.
    #             # print(f"[WARN] dynamic_axes key '{name}' not found in example_inputs, skipping")
    #             continue

    #         tensor = example_inputs[name]

    #         # Only tensors (or wrapped ints) should be used as keys in ShapesCollection
    #         if not isinstance(tensor, torch.Tensor):
    #             # If it's an int or something else, you’d need _IntWrapper logic
    #             # from the docs. For now, we skip non-tensors.
    #             # print(f"[WARN] example_inputs['{name}'] is not a Tensor, skipping")
    #             continue

    #         # axes_spec: Dict[int, str] → we build a dict {dim_idx: Dim}
    #         dim_mapping: Dict[int, Any] = {}
    #         for dim_idx, dim_name in axes_spec.items():
    #             if isinstance(dim_name, str):
    #                 dim_mapping[dim_idx] = get_dim(dim_name)
    #             else:
    #                 # If already a Dim/int, just propagate
    #                 dim_mapping[dim_idx] = dim_name

    #         # Assign this mapping to the ShapesCollection for this tensor
    #         # Using dict {dim_index: Dim} form, as allowed by ShapesCollection
    #         shapes[tensor] = dim_mapping
