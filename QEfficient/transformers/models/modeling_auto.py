...
class QEFFTransformersBase(QEFFBaseModel):
    ...
    def __init__(self, model: nn.Module, pretrained_model_name_or_path: str, **kwargs) -> None:
        ...
        self.model_card_name = model_card_name if model_card_name else (self.pretrained_model_name_or_path
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, *args, **kwargs):
        ...
        model_card_name = kwargs.pop("model_card_name", None)  # Remove model_card_name from kwargs for transformers APIs

        model = QEFFAutoModelToTransformersAutoModelMap[cls.__name__].from_pretrained(pretrained_model_name_or_path, *args, **kwargs)
        return cls(model, pretrained_model_name_or_path=pretrained_model_name_or_path, model_card_name=model_card_name, **kwargs)
...
class QEFFAutoModelForCausalLM(QEFFTransformersBase):
    ...
    def compile(self, num_cores: int, device_group: List[int], model_card_name: Optional[str] = None, batch_size: int = 1, prompt_len: int = 32, ctx_len: int = 128,
                mxfp6: bool = True, mxint8: bool = False, mos: int = -1, aic_enable_depth_first: bool = False) -> str:
        ...
        qpc_dir_path = get_qpc_dir_path(model_card_name=self.model_card_name, num_cores=num_cores, mos=mos, batch_size=batch_size,
                                        prompt_len=prompt_len, ctx_len=ctx_len, mxfp6=mxfp6, mxint8=mxint8, device_group=device_group)

        # Compile
        QEfficient.compile(onnx_path=self.onnx_path, qpc_path=os.path.dirname(qpc_dir_path),
    ...
    def run_cloud_ai_100(self, prompts: List[str],  **kwargs):
        ...
        assert self.device_id is not None, "please pass valid device_id as input argument"  # FIXME: replace with isinstance
        generation_len = kwargs.pop("generation_len", None)
        batch_size, ctx_len = get_compilation_dims(qpc_path=self.qpc_path)
        return QEfficient.cloud_ai_100_exec_kv(batch_size, self.tokenizer, self.qpc_path, prompt=prompts, device_id=self.device_id, generation_len=generation_len, ctx_len=ctx_len)
