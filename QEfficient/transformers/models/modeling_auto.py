...
class QEFFAutoModelForImageTextToText(QEFFTransformersBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if model.config.architectures[0] == "InternVLChatModel":
            ...
            self.num_key_value_heads = model.config.llm_config.num_key_value_heads
            self.head_dim = model.config.llm_config.hidden_size // model.config.llm_config.num_attention_heads
            self.pad_token_id = model.config.llm_config.pad_token_id
            self.ctx_len = Constants.CTX_LEN_VLM_INTERN
        elif model.config.architectures[0] == "LlavaForConditionalGeneration":
            breakpoint()
            self.model.config.use_cache = True
            self.processor = processor
            self.num_layers = model.config.text_config.num_hidden_layers
            self.num_key_value_heads = model.config.text_config.num_key_value_heads
            self.head_dim = model.config.text_config.hidden_size // model.config.text_config.num_attention_heads
            # self.qpc_session = qpc_session
            self.pad_token_id = model.config.pad_token_id
            self.ctx_len = Constants.CTX_LEN_VLM_LLAVA
        self.continuous_batching = continuous_batching
        self.is_tlm = is_tlm
        self.generate_func = {
            "LlavaForConditionalGeneration": self.generate_inputs_llava,
            "InternVLChatModel": self.generate_inputs_intern,
        }
    ...
    def generate_inputs(self, **kwargs):
        generate_func = self.generate_func.get(self.model.config.architectures[0])
        if not generate_func:
            raise ValueError(f"Input generation function for model {self.model.config.architectures} not found.")
    ...
    def generate_inputs_intern(self, **kwargs):
        ...
        for i in range(1, 2):
            ...
            pixel_values.append(self.processor.load_image(img, max_num=12))

        question = "<image>\nPlease describe the image in detail."

        pixel_values = torch.cat(pixel_values, dim=0)

        query = self.processor(self.processor.tokenizer, pixel_values, question)

        inputs = dict(self.processor.tokenizer(query, return_tensors="pt"))
    ...
    def export(
        ...
        **kwargs,
        ...
        example_inputs, dynamic_axes, output_names = self.generate_inputs(**kwargs)
    ...
    def cloud_ai_100_vlm_generate(
        ...
        inputs["attention_mask"] = torch.nn.functional.pad(
            inputs["attention_mask"], (0, 3072 - input_ids_size), "constant", 0
        )
    ...
    def pytorch_vlm_generate(
        ...
        for i in range(self.num_layers):
            inputs["past_key_values"].append(
                (
                    torch.zeros(self.batch_size, self.num_key_value_heads, self.ctx_len, self.head_dim),
                    torch.zeros(self.batch_size, self.num_key_value_heads, self.ctx_len, self.head_dim),
                )
            )
