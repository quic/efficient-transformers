...
def get_decoder_layer_classes_for_export(model: nn.Module) -> set:
    ...
    based on the model's architecture using the existing KVCacheTransform mapping.
    """
    DECODER_LAYER_PATTERNS = ["DecoderLayer", "Block"]

    decoder_layer_classes = set()

    for module in model.modules():
        class_name = module.__class__.__name__
        if any(pattern in class_name for pattern in DECODER_LAYER_PATTERNS):
            decoder_layer_classes.add(module.__class__)

    return decoder_layer_classes
