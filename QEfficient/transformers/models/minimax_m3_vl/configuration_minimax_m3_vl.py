...
class MiniMaxM3VLTextConfig(PreTrainedConfig):
    ...
    def __post_init__(self, **kwargs):
        ...
        if self.layer_types is None and "sparse_attention_freq" in sparse_cfg:
            # transformers 5.5 validates layer types against a shared allow-list
            # that does not include the model-specific token "minimax_m3_sparse".
            # Use generic "sparse" for compatibility and map both in attention.
            self.layer_types = ["sparse" if f else "full_attention" for f in sparse_cfg["sparse_attention_freq"]]
