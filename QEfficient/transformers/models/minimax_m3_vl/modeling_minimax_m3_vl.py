...
def _minimax_layerwise_window(total_layers: int) -> tuple[int, int, bool]:
    ...
    return start, min(end, total_layers), True


def _minimax_is_last_layer_window(total_layers: int) -> bool:
    _, end, is_active = _minimax_layerwise_window(total_layers)
    return not is_active or end >= total_layers


def _minimax_local_layer_idx(layer_idx: int) -> int:
    start, _, is_active = _minimax_layerwise_window(layer_idx + 1)
    return layer_idx - start if is_active else layer_idx


def _qeff_rotate_half(hidden_states: torch.Tensor, rotary_dim: int) -> torch.Tensor:
    half_dim = rotary_dim // 2
    first_half = hidden_states[..., :half_dim]
    second_half = hidden_states[..., half_dim:rotary_dim]
    return torch.cat((-second_half, first_half), dim=-1)


def _qeff_apply_rotary_pos_emb(
...
class QEffMiniMaxM3VLAttention(MiniMaxM3VLAttention):
    def forward(
        ...
        **kwargs,
        ...
        query_states, key_states = _qeff_apply_rotary_pos_emb(query_states, key_states, cos, sin, rotary_dim)
        ...
        if past_key_values is not None:
            ...
            layer_idx = _minimax_local_layer_idx(self.layer_idx)
        ...
        if self.indexer is not None:
            ...
            if past_key_values is None and not torch.onnx.is_in_onnx_export():
                block_indices = self.indexer(hidden_states, position_embeddings, None, position_ids)
            else:
...
class QEffMiniMaxM3VLTextModel(MiniMaxM3VLTextModel):
    def forward(
        ...
        **kwargs,
        ...
        total_layers = min(len(self.layers), self.config.num_hidden_layers)
        start, end, _ = _minimax_layerwise_window(total_layers)
...
class QEffMiniMaxM3VLForCausalLM(MiniMaxM3VLForCausalLM):
    ...
    def forward(
        ...
        **kwargs,
        ...
        total_layers = min(len(self.model.layers), self.model.config.num_hidden_layers)
        if not _minimax_is_last_layer_window(total_layers):
