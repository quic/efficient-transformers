...
class QEffDeepseekV3Attention(nn.Module):
    ...
    def fused_forward_h_blocking(
        ...
        **kwargs,
        ...
        if compressed_kvs is not None:
            kva = compressed_kvs.update_ckv(kva, self.layer_idx % 1, cache_kwargs)

        cos, sin = self.rotary_emb(kva, seq_len=32 * 1024)
        q_pe, k_pe = orig_apply_rotary_pos_emb(q_pe, k_pe, cos, sin, position_ids)

        if compressed_kvs is not None:
            k_pe = compressed_kvs.update_k_pe(k_pe, self.layer_idx % 1, cache_kwargs)
    ...
    def fused_forward_kv_blocking(
        ...
        **kwargs,
        ...
        ## Write Only
        if compressed_kvs is not None:
            compressed_kvs.write_only_ckv(kva, self.layer_idx % 1, cache_kwargs)

        cos, sin = self.rotary_emb(hidden_states, seq_len=32 * 1024)
        q_pe, k_pe = orig_apply_rotary_pos_emb(q_pe, k_pe, cos, sin, position_ids)

        if compressed_kvs is not None:
            compressed_kvs.write_only_k_pe(k_pe, self.layer_idx % 1, cache_kwargs)
    ...
    def forward_full_kv(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx % 1, cache_kwargs)
...
class QEffDeepseekV3Model(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        ctx_len = compressed_kvs.layers[start % 1].ckv.shape[-2]
        ...
        if QEffDeepseekV3Model._end == 61:
