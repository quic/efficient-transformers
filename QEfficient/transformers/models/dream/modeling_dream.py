...
class QEffDreamBlockedAttention(DreamAttention):
    """
    This is blocked attention. Set num_head_blocks, kv blocks as the environment variable
    """
    def __qeff_init__(self):
        self.rotary_emb = QEffDreamRotaryEmbedding(config=self.config)
        self.compile_length = self.config.max_position_embeddings
...
class QEffDreamSampler(nn.Module):
...
class QEffDreamModel(nn.Module):
    def __qeff_init__(self, config):
        self.vocab_size = config.vocab_size
        self.mask_token_id = config.mask_token_id
        self.sampler = QEffDreamSampler(mask_token_id = self.mask_token_id)
