...
class QEffInternVLModel(nn.Module):
    ...
    def get_dummy_inputs(
        ...
        vision_inputs = {}
        lang_inputs = {}
        vision_inputs["pixel_values"] = torch.zeros((inputs_shapes["pixel_values"]), dtype=self.config.vision_config.torch_dtype)
        lang_inputs["input_ids"] = torch.zeros((inputs_shapes["input_ids"]), dtype=torch.int64)
        lang_inputs["vision_embeds"] = torch.zeros((inputs_shapes["vision_embeds"]), dtype=self.config.vision_config.torch_dtype)
        ...
        for i in range(self.language_model.config.num_hidden_layers):
            for kv in ["key", "value"]:
                lang_inputs["past_key_values"][i].append(torch.zeros(kv_cache_shape, dtype=self.config.llm_config.torch_dtype))
    ...
    def get_inputs_info(self):
        return [
            ...
            IOInfo(name="pixel_values", datatype=self.config.vision_config.torch_dtype, shape=("num_patches", 3, "img_size", "img_size")),
        ]
