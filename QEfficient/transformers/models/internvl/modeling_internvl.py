...
class QEffInternVLModel(nn.Module):
    ...
    def get_specializations(self, batch_size: int, prefill_seq_len: int, ctx_len: int, img_size: int, **compiler_options):
    ...
    def generate_dummy_inputs(self, kv_offload: bool = False):
        ...
        inputs_shapes['position_ids'] = (constants.ONNX_EXPORT_EXAMPLE_BATCH_SIZE, constants.ONNX_EXPORT_EXAMPLE_SEQ_LEN)
        inputs_shapes['pixel_values'] = (NUM_CROPS, C, H, W)
        
        # Define inputs
        inputs = {}
        inputs['input_ids'] = torch.zeros((inputs_shapes['input_ids']), dtype=torch.int64)
        inputs['position_ids'] = torch.arange(constants.ONNX_EXPORT_EXAMPLE_SEQ_LEN, dtype=torch.int64).view(1, constants.ONNX_EXPORT_EXAMPLE_SEQ_LEN).repeat(constants.ONNX_EXPORT_EXAMPLE_BATCH_SIZE, 1)
        inputs['pixel_values'] = torch.zeros((inputs_shapes['pixel_values']), dtype=torch.float32)
        
        # Add data for KV
        kv_cache_shape = get_padding_shape_from_config(config=self.language_model.config, batch_size=constants.ONNX_EXPORT_EXAMPLE_BATCH_SIZE, seq_len=constants.ONNX_EXPORT_EXAMPLE_SEQ_LEN)
