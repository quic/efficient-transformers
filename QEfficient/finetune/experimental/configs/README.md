...
Here’s a concise README snippet for your **dataset & dataloader configuration** arguments:
...
*  **gradient_checkpointing_kwargs** :
   ...
   *  **preserve_rng_state**: `True` → Controls whether to preserve the RNG (Random Number Generator) state during checkpointing. Preserving RNG state ensures reproducibility of stochastic operations (e.g., dropout) when recomputing activations during backward passes.

   *  **use_reentrant**: `False`  → Determines whether to use reentrant gradient checkpointing. Reentrant checkpointing uses PyTorch's built-in mechanism for recomputation, which can reduce memory usage but may have limitations with certain custom autograd functions.
