...
class QEFFBaseModel(ABC):
    ...
    def _compile(
        ...
        **compiler_options,
        ...
        # Silently discard any stale mdp_dump_partition_config key that callers
        # may still pass; the compiler-round-trip dump path is no longer supported.
        compiler_options.pop("mdp_dump_partition_config", None)
        mdp_ts_json = None
        ...
        if mdp_ts_json_path:
            ...
            # round-trip required.
            from QEfficient.compile.mdp_generator import generate_disagg_mdp_partition_config

            num_layers = getattr(self, "num_layers", None)
            if num_layers is None:
                raise AttributeError("Model does not expose 'num_layers'. Cannot generate disagg MDP partition config.")
            num_cores = compiler_options.get("aic_num_cores", constants.DEFAULT_AIC_NUM_CORES)
            logger.info(
                f"Generating disagg MDP partition config from ONNX: "
