...
class RewriteUnsupportedOpsTransform(BaseOnnxTransform):
    """Replace CastLike→Cast and SplitToSequence→Split (unsupported by QAIC compiler)."""
    ...
    @classmethod
    def _extract_const_value(cls, arr, const_map, name):
        if arr.ndim == 0:
    ...
    @classmethod
    def _extract_constant_node(cls, node, const_map):
    ...
    @classmethod
    def _rewrite_container(cls, container, type_map, const_map):
    ...
    @classmethod
    def _try_rewrite_split(cls, node, nodes, output_to_node, const_map, new_nodes, replace_map, skip_nodes):
        ...
        if len(node.input) > 1 and node.input[1]:
            ...
            split_attr = split_val if isinstance(split_val, list) else ([split_val] * num_outputs if isinstance(split_val, int) else None)
            if split_attr:
                sc = f"{node.name or split_out}_split_sizes"
                new_nodes.append(onnx.helper.make_node("Constant", [], [sc], name=f"{sc}_const",
                    value=onnx.helper.make_tensor(sc, TensorProto.INT64, [len(split_attr)], split_attr)))
                split_inputs.append(sc)
            else:
                split_inputs.append(node.input[1])
        new_nodes.append(onnx.helper.make_node("Split", split_inputs, split_outputs,
            name=f"{node.name}_split" if node.name else "", axis=axis))
        for seq_at in consumers:
    ...
    @classmethod
    def _create_cast_node(cls, node, output_to_node, type_map):
    ...
    @classmethod
    def _apply_replacements(cls, container, new_nodes, replace_map, skip_nodes):
    ...
    @classmethod
    def _get_const_int(cls, name, output_to_node, const_map):
    ...
    @classmethod
    def _resolve_type(cls, name, output_to_node, type_map):
        if name in type_map:
    ...
    @classmethod
    def _infer_function_const_inputs(cls, model, const_map):
        func_const_map: Dict[str, Dict] = {}
        for fn in model.functions:
            if not fn.input:
                continue
            per_input: Dict = {}
    ...
    @classmethod
    def _infer_function_const_outputs(cls, model):
        func_output_consts: Dict = {}
        for fn in model.functions:
            output_map: Dict = {}
...
class SplitMultiInputMaxMinTransform(BaseOnnxTransform):
    ...
    The dynamo ONNX exporter emits Max(a, b, c) via onnxscript's _aten_index_onnx
    helper when aten.index_put is called with 3 tensor indices. QAIC only supports
    2-input Max/Min, so this transform rewrites them as Max(Max(a, b), c).
    """
        ...
        for node in model.graph.node:
            if node.op_type in ("Max", "Min") and len(node.input) > 2:
                ...
                for i in range(1, len(inputs)):
                    ...
                    out = final_output if is_last else f"{final_output}_split_{i}"
                    new_nodes.append(onnx.helper.make_node(
                        node.op_type, inputs=[intermediate, inputs[i]], outputs=[out],
                        name=f"{node_name}_split_{i}"))
                    intermediate = out
                transformed = True
            else:
                new_nodes.append(node)
...
class OnnxTransformPipeline(BaseOnnxTransform):
    ...
    def apply(
        ...
        **kwargs,
        ...
        if SplitMultiInputMaxMinTransform in requested:
            applied[SplitMultiInputMaxMinTransform] = SplitMultiInputMaxMinTransform.apply(model)
