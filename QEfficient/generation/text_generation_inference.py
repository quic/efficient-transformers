...
class QEffTextGenerationBase:
    ...
    def run_continuous_batching_decode(self, prompt_queue, generation_len):
        ...
        while prompt_queue or current_decode_ongoing.any():
            ...
            # Update CCL and comp_ctx_len based on the maximum position id
            if max_position_id >= CCL - 1:
                ccl_id += 1
    ...
    def run_decode(self, decode_inputs, generation_len, streamer: Optional[transformers.TextStreamer] = None):
        ...
        decode_time = []
        # for different ccl we count the number of generated tokens for the performance purposes
        current_token_idx_ccl = 0
        for num_token in range(1, generation_len):
            if cache_index >= CCL - 1:
                # performance
                ccl_end = perf_counter()
                throughput = current_token_idx_ccl / (ccl_end - ccl_start)
