...
def _build_meta_qeff_model(qeff_model):
    ...
    source_model = getattr(qeff_model.model, "model", qeff_model.model)
    with init_empty_weights():
        try:
            meta_model = qeff_model._hf_auto_class.from_config(config, attn_implementation="eager")
        except ValueError:
            meta_model = source_model.__class__(config)

    if qeff_model.__class__.__name__ == "QEffCausalLMForTextImageToTextModel" and hasattr(meta_model, "vision_tower"):
        meta_model.vision_tower = nn.Identity()

    if quant_config is None:
        target_dtype = getattr(config, "dtype", None) or getattr(config, "torch_dtype", None) or torch.float32
        if target_dtype == torch.bfloat16:
            target_dtype = torch.float16
        meta_model = meta_model.to(dtype=target_dtype)

    if not hasattr(meta_model, "get_qeff_language_decoder") and not hasattr(meta_model, "get_qeff_vision_encoder"):
        from QEfficient.transformers.models.pytorch_transforms import KVCacheExternalModuleMapperTransform

        meta_model, _ = KVCacheExternalModuleMapperTransform.apply(meta_model)
...
def _find_checkpoint_key(
    ...
    if stripped.startswith("model."):
        without_wrapper = stripped[len("model.") :]
        if without_wrapper in checkpoint_index:
            return without_wrapper
...
def _promote_initializers_and_build_spec(onnx_program, model_ref: str, model_name: str, qeff_model) -> WeightSpec:
    ...
    for name, init_value in list(model_ir.graph.initializers.items()):
        ...
        if checkpoint_key is None and name in model_names:
            checkpoint_key = _find_checkpoint_key(name, checkpoint_index, backbone)
...
def _set_module_tensor(module: nn.Module, tensor_name: str, tensor: torch.Tensor) -> None:
    parts = tensor_name.split(".")
    parent = module
    for part in parts[:-1]:
        parent = parent[int(part)] if part.isdigit() else getattr(parent, part)
    leaf_name = parts[-1]
    existing = getattr(parent, leaf_name)
    if isinstance(existing, torch.nn.Parameter):
        setattr(parent, leaf_name, torch.nn.Parameter(tensor, requires_grad=existing.requires_grad))
    else:
        setattr(parent, leaf_name, tensor)


def _materialize_checkpoint_metadata_tensors(qeff_model, model_ref: str) -> None:
    checkpoint_files = _resolve_checkpoint_files(model_ref)
    checkpoint_index = _load_checkpoint_index(checkpoint_files)
    backbone = qeff_model.model.base_model if isinstance(qeff_model.model, PooledModel) else qeff_model.model
    tensors = list(qeff_model.model.named_parameters()) + list(qeff_model.model.named_buffers())
    for name, tensor in tensors:
        if not name.endswith("weight_shape") or not getattr(tensor, "is_meta", False):
            continue
        checkpoint_key = _find_checkpoint_key(name, checkpoint_index, backbone)
        if checkpoint_key is None:
            continue
        ...
        with safe_open(checkpoint_file, framework="pt", device="cpu") as handle:
            concrete_tensor = handle.get_tensor(checkpoint_key)
        _set_module_tensor(qeff_model.model, name, concrete_tensor)


def export_loaded_model_weight_free_onnx(
    qeff_model,
    tmp_onnx_path: Path,
    ...
    """Export an already-transformed model, then externalize its initializers.

    This Kimi fallback avoids compressed-tensors meta tracing limitations while
    still producing a weight-free ONNX plus weight_spec.json.
    """
    ...
    if onnx_program is None:
        raise RuntimeError("torch.onnx.export returned None for loaded-model weight-free export")

    checkpoint_dir = tmp_onnx_path.parent / "loaded_weight_checkpoint"
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_path = checkpoint_dir / "model.safetensors"
    state_dict = {
        name: tensor.detach().cpu().contiguous().clone()
        for name, tensor in qeff_model.model.state_dict().items()
    ...
    save_weight_spec(tmp_onnx_path.with_name("weight_spec.json"), spec)
    onnx_program.save(str(tmp_onnx_path))

    def _cleanup():
        return None

    return qeff_model, onnx_transform_kwargs, _cleanup
...
def export_weight_free_onnx(
    ...
    _materialize_checkpoint_metadata_tensors(meta_qeff_model, model_ref)
    ...
    with export_context:
        ...
        target_dtype = (
            getattr(qeff_model.model.config, "dtype", None)
            or getattr(qeff_model.model.config, "torch_dtype", None)
            or torch.float32
        )
        if target_dtype == torch.bfloat16:
            target_dtype = torch.float16
