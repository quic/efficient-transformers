...
def CtxScatter(data: onnxscript.FLOAT, position_ids: onnxscript.INT32, updates: onnxscript.FLOAT) -> onnxscript.FLOAT:
    # Find dims
    batch_size = ops.Reshape(ops.Gather(ops.Shape(data), [0]), ops.Constant(value_ints=[]))
    num_heads = ops.Reshape(ops.Gather(ops.Shape(data), [1]), ops.Constant(value_ints=[]))
    seq_len = ops.Reshape(ops.Gather(ops.Shape(position_ids), [1]), ops.Constant(value_ints=[]))

    # Expanded shape to create indices
    zero = ops.Reshape(ops.Cast(ops.Constant(value_ints=[0]), to=7), ops.Constant(value_ints=[]))  # Scalar INT64
    one_scalar = ops.Reshape(ops.Cast(ops.Constant(value_ints=[1]), to=7), ops.Constant(value_ints=[]))  # Scalar INT64
    one_vec = ops.Cast(ops.Constant(value_ints=[1]), to=7)  # 1D tensor [1] INT64
    exp_shape = ops.Concat(
        ops.Unsqueeze(batch_size, [0]), ops.Unsqueeze(num_heads, [0]), ops.Unsqueeze(seq_len, [0]), one_vec, axis=0
    )

    # Create indices
    batch_idx = ops.Expand(ops.Unsqueeze(ops.Range(zero, batch_size, one_scalar), [1, 2, 3]), exp_shape)
    head_idx = ops.Expand(ops.Unsqueeze(ops.Range(zero, num_heads, one_scalar), [0, 2, 3]), exp_shape)
    ctx_idx = ops.Expand(ops.Unsqueeze(ops.Cast(position_ids, to=7), [1, 3]), exp_shape)  # Cast position_ids to INT64
...
def CtxScatter3D(data: onnxscript.FLOAT, position_ids: onnxscript.INT32, updates: onnxscript.FLOAT) -> onnxscript.FLOAT:
    ...
    one_scalar = ops.Reshape(ops.Cast(ops.Constant(value_ints=[1]), to=7), ops.Constant(value_ints=[]))  # Scalar INT64
    one_vec = ops.Cast(ops.Constant(value_ints=[1]), to=7)  # 1D tensor [1] INT64
    exp_shape = ops.Concat(ops.Unsqueeze(batch_size, [0]), ops.Unsqueeze(seq_len, [0]), one_vec, axis=0)

    # Create indices
    batch_idx = ops.Expand(ops.Unsqueeze(ops.Range(zero, batch_size, one_scalar), [1, 2]), exp_shape)
    ctx_idx = ops.Expand(ops.Unsqueeze(ops.Cast(position_ids, to=7), [2]), exp_shape)  # Cast position_ids to INT64
...
def CtxGather3D(data: onnxscript.FLOAT, ctx_indices: onnxscript.INT32) -> onnxscript.FLOAT:
    ...
    ctx_indices = ops.Cast(ctx_indices, to=7)
