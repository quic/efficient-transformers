...
class CtxScatterFunc(torch.autograd.Function):
    ...
    def symbolic(
        ...
        return g.onnxscript_op(CtxScatter, data, batch_index, position_ids, updates).setTypeAs(data)


@onnxscript.script(onnxscript.values.Opset("com.qualcomm.cloud", 1))
def CtxGather(data: onnxscript.FLOAT, batch_index: onnxscript.INT32, ctx_indices: onnxscript.INT32) -> onnxscript.FLOAT:
    batch_size = ops.Gather(ops.Shape(batch_index), [0])
    num_heads = ops.Gather(ops.Shape(data), [1])
...
class CtxGatherFunc(torch.autograd.Function):
    ...
    def symbolic(g: torch.Graph, data: torch.Value, batch_index: torch.Value, ctx_indices: torch.Value) -> torch.Value:
        return g.onnxscript_op(CtxGather, data, batch_index, ctx_indices).setTypeAs(data)
