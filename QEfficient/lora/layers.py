...
class LinearMultiLoRA(nn.Linear):
    ...
    def multilora_init(self, lora_rank, max_num_adapters):
        ...
        self.lora_weight_A = nn.Parameter(self.weight.new_zeros(self.max_num_adapters, 1, self.in_features, self.lora_rank))
        self.lora_weight_A.requires_grad = False
        self.lora_weight_B = nn.Parameter(self.weight.new_zeros(self.max_num_adapters, 1, self.lora_rank, self.out_features))
