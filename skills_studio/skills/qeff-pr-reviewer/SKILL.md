...
>    - **Custom torch/ONNX op or its registration.** Belongs under `QEfficient/customop/` and is re-exported from `QEfficient/customop/__init__.py` (the public custom-op surface). A custom-op definition or op-registry mutation placed in `base/modeling_qeff.py` or a modeling file is misplaced = Issue.
...
> - **Tensor materialized just to read metadata** — `x.detach().clone().shape`, `x.clone().shape[-1]`, `int(t.detach().clone().shape[...])` where the result feeds only a shape/`.size()`/`.numel()` read. **Silent unless** the `.detach()`/`.clone()` output is used solely for a scalar read — then the copy is pure waste (`int(self.rotary_emb.cos_cached.shape[-1])` is the correct form; the clean sibling exists in-tree). Issue. Do NOT flag `.detach()`/`.clone()` whose output is used as a real tensor — those are legitimate and common (deepseek_v3 alone has ~9).
