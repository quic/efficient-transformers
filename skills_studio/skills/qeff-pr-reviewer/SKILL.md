...
**Precision contract (every subagent must clear this bar before emitting a finding).** This skill is judged on two axes at once: *find every real issue* AND *never ship a junk comment*. A false positive costs more than a missed nit — it trains the author to ignore the whole review. So a finding ships only if you can name its concrete consequence: the input/state that yields a wrong result, a crash, a poisoned cache, a broken/OOM export, a violated **cited** convention (quote the `repo_conventions.md` line or the in-tree precedent), a CONTRIBUTING.md requirement, or wasteful compute/allocation that clears one of the Subagent C perf guards below (name the wasted work concretely). If the honest answer is "style preference, no failure mode, no convention behind it," drop it or mark it Nit and move on. **Never emit:** (a) "consider renaming for clarity" without a concrete misleading-name failure at a call site; (b) "this could be simplified/cleaner" without naming the exact redundant code and what it duplicates; (c) "why did you change X?" scope questions you can't tie to a layering or contract violation; (d) any perf/redundancy flag whose silent-unless guard (Subagent C) is not met; (e) a repo-convention claim you have not verified against the actual tree — grep for the symbol/dir first (`select_interface`, `MAX_SEQ_LEN`, etc. may be PR-internal and not exist on main; a finding built on a hallucinated convention is itself junk). When two findings cite the same root cause, emit one, not two.
...
> You are the QEfficient maintainer reviewing PR diff `/tmp/pr<N>.diff`. Your scope is **design correctness for this repo only** — layering/change-placement (is each change at the right layer, or did model-specific logic leak into a base class?), symbol/directory placement, whether each new file is needed, Auto class contract, transform registration, hash plumbing, ONNX transform contract, PrefillOnly/Revert symmetry, CtxScatter/CtxGather variants, `QEff*.__init__` override rule, cross-class state coupling, hallucinated transformers imports.
>
...
>
> 12. **Symbol / directory placement** (`#file-and-symbol-placement`). Read that appendix; it lists the verified homes (constants, custom ops, test images, family-specific constants). For each new symbol the PR introduces, check it landed in the right place:
>    - **New magic number / constant.** A model-family-specific value (a ctx-len, seq-len, cap, or magic threshold for one model) belongs in `QEfficient/utils/constants.py` following the `<FAMILY>_<NAME>` convention already there (`INTERN_CTX_LEN`, `FLUX_ONNX_EXPORT_SEQ_LENGTH`, `WAV2VEC2_MAX_SEQ_LEN`, etc.), NOT hardcoded inline in the modeling/example file. Inline magic number that should be a named constant = Issue. **Before recommending "reuse existing constant X", grep `constants.py` to confirm X exists** — there is no generic `MAX_SEQ_LEN`; naming a constant that isn't there is a junk finding.
>    - **Custom torch/ONNX op or its registration.** Belongs under `QEfficient/customop/` and is re-exported from `QEfficient/customop/__init__.py` (the public custom-op surface). A custom-op definition or op-registry mutation placed in `base/modeling_qeff.py` or a modeling file is misplaced = Issue. **MUST FIRE when** the diff adds `import QEfficient.customop.<x>` (or similar side-effect import that registers `torch.ops.qefficient.*`) anywhere outside `QEfficient/customop/__init__.py` — the `# noqa: F401` marker does NOT excuse placement; a side-effect import for op registration belongs in the customop package's own `__init__.py` (or is invoked from it), not in `base/modeling_qeff.py` or a top-level module. The reviewer's job is to say "this import is real but should move," not "it has `noqa: F401` so it's fine."
>    - **Test image.** The repo keeps a test-image bank at `docs/image/` (`Cloud_AI_100.png`, `girl_laughing.png`). A new test that downloads an image at runtime, inlines base64, or adds a new image elsewhere should use `docs/image/` instead = Nit → Issue if it adds a redundant image file.
>    - Do NOT invent placement rules. If you are unsure where something belongs, grep for a sibling of the same kind and cite it; if you can't find one, don't flag placement.
>
> 13. **Is this new file needed?** For **every added file** in the diff (`grep -E "^diff --git" | ...` or the Phase-2 file list), ask three questions and flag only if the answer is clearly wrong: (a) could this content live in an existing sibling file instead of a new one? (b) is the file on the code path this PR changes, or is it dead scaffolding (a helper nothing imports, a config nothing loads, a second copy of an existing file)? — grep the new module's basename across the whole tree (not just the diff) to confirm something imports it; (c) does its content match the directory's stated purpose (`repo_conventions.md#top-level-layout`)? A new file that nothing references = Issue (dead code) → Blocker if it's a duplicate of an existing file. A file whose content belongs in an existing module = Issue. **EXEMPT `examples/`, `tests/`, `scripts/`, and `docs/` from the import-reference test** — these are entrypoints (pytest collection, CLI `__main__`, Sphinx), legitimately never imported; being unimported is expected, not dead code. Apply the "nothing imports it" test only to files under `QEfficient/` meant to be imported. **Silent unless** you have grepped the whole tree and confirmed the file is unreferenced or duplicated — do not flag a new file merely for existing.
>
...
>
> **Perf / redundancy smells (high-value miss class — each has a SILENT-UNLESS guard; do not flag the bare pattern).** These are the critiques human reviewers make most and the skill has historically missed, but they are also the easiest to turn into junk. Emit ONLY when the specific guard is met:
> - **Tensor materialized just to read metadata** — `x.detach().clone().shape`, `x.clone().shape[-1]`, `int(t.detach().clone().shape[...])` where the result feeds only a shape/`.size()`/`.numel()` read. **MUST FIRE when** the diff contains the literal patterns `.detach().clone().shape` or `.clone().shape[` and the immediately-enclosing expression is `int(...)`, an assignment consumed only by `.shape`/`.size()`/`.numel()`, or a scalar arithmetic on such — e.g. `rotary_dim = int(self.rotary_emb.cos_cached.detach().clone().shape[-1])` (a real in-tree waste site). **Silent only if** you can point to the same expression being *also* assigned to a variable, returned, or wrapped in `nn.Parameter` — otherwise fire. Issue. The clean form is `int(self.rotary_emb.cos_cached.shape[-1])`. Do NOT flag `.detach()`/`.clone()` whose output is unambiguously used as a real tensor (fed to a matmul, added to another tensor, stored on `self`) — those are legitimate and common (deepseek_v3 alone has ~9).
> - **`copy.deepcopy` of a live model / large module in a hot or init path** — **silent unless** the deepcopy target is a full model/pipeline/`nn.Module` on a live (materialized) path AND the copy's result is subsequently discarded or fully overwritten by transforms (e.g. `self.model = copy.deepcopy(model)` right after `super().__init__(model)`, so transforms mutate a throwaway). Issue → Blocker if it doubles export RAM. Do NOT flag: `deepcopy` of a small dict/config (`copy.deepcopy(kwargs)`, `copy.deepcopy(qeff_model.hash_params)` are the established safe idiom); a deepcopy of a meta-device / `init_empty_weights` model (zero real memory); or a copy made specifically to avoid mutating the caller's model before an in-place op like `tie_weights()` (e.g. `quantizer_utils.py:51`).
> - **Redundant kwarg at its documented default** — a call passing `dtype=torch.float32` / a flag whose value equals the callee's default. **Silent unless** (a) you have read the callee's signature in the pinned `transformers==5.5.4` / torch version and confirmed the param exists with that exact default, AND (b) the call is NOT in a `forward()` override, `get_dummy_inputs`, or export/compile glue. Passing explicit flags (`use_cache=True`, `return_dict=True`, `dtype=`) to pin graph/export behavior against upstream HF default-drift is the **established QEff idiom** (~40 deliberate sites), not waste — and in `transformers==5.5.4`, `LlamaForCausalLM.forward` has no `return_dict` param and `use_cache` defaults to `None`, so the "obvious" defaults are often wrong. If either guard is unmet, drop it. Nit → Issue only if verified redundant AND it obscures intent.
> - **Redundant loop / recomputation** — iterating the same collection N times where one pass suffices, or recomputing an invariant inside a loop. **Silent unless** you can state the single-pass rewrite concretely. Issue.
> - **Mass-allocation waste** — `np.random.rand(...)` / `np.ones(...)` / `np.zeros(...)` to allocate a buffer whose values are **provably overwritten before they are read** (allocated, then fully assigned on the next lines → use `np.empty`), or a full-tensor copy where a view/slice suffices. **Silent unless** the overwrite is visible in the diff. Do NOT flag an all-ones attention mask, an all-ones mask row, or any array whose `1`s/`0`s are the intended data (`np.ones(...)` as a mask is the value, not scratch). Issue.
>
> **Code-shape / redundancy (dead or pointless structure — verifiable, low false-positive):**
> - **Override that only calls super and returns** — `def apply(cls, model): model, t = super().apply(model); return model, t`, or any method whose body is `return super().<same>(...)`. Redundant only when the override's signature is **identical** to the parent's AND it is **unconditional** AND it forwards **every argument unchanged**. A real in-tree deletable example is `pytorch_transforms.py:924`. Issue. **Do NOT flag** an override that narrows/renames/drops/injects a parameter (`get_seq_length` dropping `cache_position` for backward-compat, a `forward` swallowing `**kwargs`, a method that sets `kwargs["lora_ids"]` before calling super), or wraps the super-call in an `if`/`try` (conditional fallback) — those change behavior; verify against the parent signature before flagging.
> - **Dead `__init__` body** — a `QEff<X>.__init__` that only calls `super().__init__(...)`, or re-assigns attributes the class-swap transform already provides (see `repo_conventions.md#qeff-class-rules` for the override rule and its legit exceptions). If it never runs in the class-swap flow, it's dead. Issue (Blocker if it re-inits pretrained weights).
> - **Nested `try/except` / `try/finally` that flattens** — a `try` whose only content is another `try`, or a `try/finally` where the `finally` is unnecessary. Issue only when the nesting obscures the control flow; do not flag a `finally` that does real cleanup.
> - **Unused import / dead constant / commented-out block** — an import or module-level constant added by the diff with zero references anywhere, or a commented-out code block with no re-enable note. **Grep the ENTIRE tree, not just the diff** — an import in an `__init__.py` re-export is consumed by another (often unchanged) module. Skip any import carrying `# noqa: F401`, listed in `__all__`, or living in an `__init__.py` re-export file. Since ruff already auto-flags true unused imports (F401), treat a stray one as **Nit**, not Issue, unless they ship en masse (>10 = pre-commit bypassed). A commented-out block or a genuinely unreferenced module-level constant is still an Issue.
>
...
>
> Load `<SKILL_DIR>/references/repo_conventions.md` sections `## Onboarding-completeness contract`, `## Bug-fix testing contract`, `## Tests`, `## Examples`, `## Docs` first.
>
> **Tests** (the bar is **dummy-layer** parity in CI + a unit test — NOT a full-size model run):
> - **Apply the bug-fix checks when the PR title identifies a bug/regression/hotfix, a PR label identifies it as a bug fix, or the diff itself repairs existing incorrect behavior.** Do not let missing or inaccurate metadata bypass these checks; if title/label metadata is unavailable, classify from the diff.
> - **Bug-fix PRs require both reproducer coverage and a focused regression/unit test.** Verify the diff adds or updates a `RegressionScenario` in `tests/reproducer_configs/test_reported_reproducer_configs.py` for the reported failure, with the exact failing stage/options and a `tiny_model_id` using a tiny-random checkpoint or faithful 2/4-layer reduction of the affected architecture. A full official model is acceptable only when reduction cannot reproduce the bug; it must be gated by `QEFF_REPRODUCER_RUN_FULL_MODELS=1` and the PR must explain why. Missing reproducer coverage, a scenario that skips by default without an unavoidable external dependency, or a full-model-only scenario with no justification = Blocker.
> - The reproducer scenario does **not** replace the normal regression/unit test. Verify the PR also adds or updates the narrowest relevant test under `tests/unit_test/` or `tests/transformers/` and that it exercises the repaired behavior at the real failing boundary. The test must fail when the source fix is removed and pass with it; smoke-only assertions, tests of a helper that bypass the failing production path, or a test that merely restates the implementation do not count. Missing meaningful regression/unit coverage on a bug-fix PR = Blocker. Prefer an existing test module or `tests/unit_test/models/test_model_quickcheck.py`; do not require a new test file when an existing gate can carry the case.
...
> - **VLM harness only runs 2 of 4 stages.** In `tests/transformers/models/image_text_to_text/test_image_text_to_text_models.py` the KV-vs-HF and ORT-vs-HF assertions around the `export()` call are commented out — only HF-vs-AI100 runs. A new VLM whose sole evidence is this harness is NOT 4-stage validated. **MUST FIRE as Issue** when a PR adds a new VLM and relies solely on this harness for parity — the correct fix is a `test_model_quickcheck.py` entry (CPU-runnable, HF↔QEff↔ORT parity) OR uncommenting the KV/ORT asserts for the specific model. Do NOT accept "matches the known VLM-harness limitation" as sufficient — CONTRIBUTING.md explicitly asks for 4-stage parity. The addition of a quickcheck test is a required deliverable for the PR, not an optional add-on.
